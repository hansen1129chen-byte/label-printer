...
/**
 * Call Speedaf API
 * @param {string} path - API path, e.g. /order/createOrder
 * @param {object} bizData - business data (plain object or JSON string)
 * @param {object} opts
 * @param {boolean} opts.dataAsObject - true: sign & wrap data as object; false: data is a JSON string (for createOrder/print/cancel)
 */
...
/** 1. Create order → returns billCode */
async function createOrder(order, orderItems) {
  const data = {
    ...
    acceptProvinceName: order.accept_province || '',
    ...
    parcelWeight: orderItems.reduce((s, i) => s + (i.quantity || 1) * 0.4, 0),
